...
const MAX_MESSAGES = 20; // Limite de messages par conversation
  ...
  const sendMessage = async (text: string) => {
    ...
    if (userMessageCount >= MAX_MESSAGES) {
      ...
      const endMessage: ChatMessage = { 
        text: '🙏 Merci pour votre intérêt ! Cette conversation a atteint sa limite.\n\nPour continuer à échanger avec nous, n\'hésitez pas à nous contacter :\n\n📞 Téléphone & WhatsApp : +32 476 34 23 64\n📧 Email : info@gims-consulting.be\n📝 Formulaire de contact : https://app.myguidedigital.com/contact\n\nNotre équipe sera ravie de vous accompagner ! 😊', 
        sender: 'bot' 
      };
    ...
    try {
      ...
      // Vérifier si c'est le dernier message autorisé (20e)
      if (userMessageCount + 1 >= MAX_MESSAGES) {
        setConversationEnded(true);
        setTimeout(() => {
          const endMessage: ChatMessage = { 
            text: '🙏 Merci pour cette conversation ! Vous avez atteint la limite de messages.\n\nPour poursuivre, contactez-nous directement :\n\n📞 Téléphone & WhatsApp : +32 476 34 23 64\n📧 Email : info@gims-consulting.be\n📝 Formulaire de contact : https://app.myguidedigital.com/contact\n\nÀ très bientôt ! 😊', 
            sender: 'bot' 
          };
          setMessages(prev => [...prev, endMessage]);
        }, 1000);
          ...
          <div className="border-t border-gray-200 p-3 bg-white">
            {conversationEnded ? (
              <div className="text-center py-2">
                <p className="text-xs text-gray-500 mb-2">Conversation terminée</p>
                <div className="flex items-center justify-center gap-3">
                  <a
                    href="https://wa.me/32476342364"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-2 bg-green-500 text-white rounded-full text-xs font-medium hover:bg-green-600 transition-colors"
                  >
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.125.553 4.12 1.521 5.853L0 24l6.335-1.652A11.943 11.943 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.82 0-3.543-.467-5.043-1.29l-.36-.214-3.742.981.999-3.648-.235-.374A9.935 9.935 0 012 12C2 6.486 6.486 2 12 2s10 4.486 10 10-4.486 10-10 10z"/></svg>
                    WhatsApp
                  </a>
                  <a
                    href="https://app.myguidedigital.com/contact"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-primary to-pink-500 text-white rounded-full text-xs font-medium hover:shadow-lg transition-all"
                  >
